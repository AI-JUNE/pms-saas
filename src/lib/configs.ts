import { projects, phases, members, requirements, issues, tests, risks, tasks, documents, meetings, sprints, interfaces, infraAssets, firewallRequests, procurementItems, boards } from '@/db/schema';
import { RISK_TRANSFORM, DOCUMENTS_TRANSFORM, type CrudConfig } from './crud';
export const CONFIGS: Record<string, CrudConfig> = {
  ...
  phases: { table: phases, resource: 'phase', scope: 'project', codePrefix: 'PH', orderAsc: true, fields: ['name','color','sortOrder','status'], required: ['name'] },
  ...
  tasks: { table: tasks, resource: 'task', scope: 'project', codePrefix: 'WBS', orderAsc: true, fields: ['name','phase','assignee','status','startDate','endDate','progress','reqCode','predecessor','parentId','plannedHours','actualHours'], required: ['name'] },
