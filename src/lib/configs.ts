import { projects, phases, members, requirements, issues, tests, risks, tasks, documents, meetings, sprints, interfaces, infraAssets, firewallRequests, procurementItems, boards, testCycles, todos, snapshots, formDefinitions } from '@/db/schema';
...
export const CONFIGS: Record<string, CrudConfig> = {
  projects: { table: projects, resource: 'project', scope: 'org', codePrefix: 'PRJ', fields: ['name','client','orderer','contractNo','budget','startDate','endDate','status','pmUserId'], required: ['name'] },
  ...
  requirements: { table: requirements, resource: 'requirement', scope: 'project', codePrefix: 'REQ', fields: ['title','description','category','priority','status','assignee','acceptanceCriteria'], required: ['title'] },
  ...
  issues: { table: issues, resource: 'issue', scope: 'project', codePrefix: 'ISS', fields: ['title','description','type','priority','status','assignee','dueDate','labels','storyPoints','sprintId','epic','reqCode','estimateHours','spentHours','related'], required: ['title'], journal: true },
  risks: { table: risks, resource: 'risk', scope: 'project', codePrefix: 'RSK', fields: ['title','description','probability','impact','status','owner','mitigation','contingency','dueDate'], required: ['title'], transform: RISK_TRANSFORM },
  ...
  documents: { table: documents, resource: 'document', scope: 'project', codePrefix: 'DOC', fields: ['title','type','version','status','author','approver'], required: ['title'], transform: DOCUMENTS_TRANSFORM, approveOn: { field: 'status', values: ['approved','rejected'] }, versionOn: true },
  meetings: { table: meetings, resource: 'meeting', scope: 'project', codePrefix: 'MTG', fields: ['title','meetingDate','location','attendees','agenda','decisions','actionItems','nextDate'], required: ['title'] },
  ...
  formDefinitions: { table: formDefinitions, resource: 'formDefinition', scope: 'project', codePrefix: 'FORM', fields: ['name','targetType','fields','note','status'], required: ['name'] },
