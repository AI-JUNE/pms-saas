    ...
    columns={[
      ...
      {key:'minutes',label:'결정·조치',render:(_v,row)=>{
        ...
        if (!dec.length && !act.length) {
          if (past) return <span title={`회의일(${row?.meetingDate || '-'})이 지났는데 결정사항·후속조치가 기록되지 않았습니다.${agenda.length ? `\n안건 ${nfmt(agenda.length)}건은 등록되어 있습니다 — 회의록을 작성하세요.` : ''}`} style={{color:'#c0414f',fontWeight:700,cursor:'help'}}>⚠ 회의록 미작성</span>;
          return <span style={{color:'#9a9a9a',cursor:'help'}} title={agenda.length ? `예정 회의 — 안건 ${nfmt(agenda.length)}건 등록됨` : '예정 회의 — 안건 미등록'}>{agenda.length ? `안건 ${nfmt(agenda.length)}건` : '—'}</span>;
        }
        const tip = [
          agenda.length ? `안건 ${nfmt(agenda.length)}건` : '안건 미등록',
          dec.length ? `결정사항 ${nfmt(dec.length)}건\n${dec.slice(0, 5).map((d, i) => `  ${i + 1}. ${d}`).join('\n')}${dec.length > 5 ? `\n  … 외 ${nfmt(dec.length - 5)}건` : ''}` : '결정사항 없음',
          act.length ? `후속조치 ${nfmt(act.length)}건\n${act.slice(0, 5).map((a, i) => `  ${i + 1}. ${a}`).join('\n')}${act.length > 5 ? `\n  … 외 ${nfmt(act.length - 5)}건` : ''}` : '후속조치 없음',
        ].join('\n');
        return (
          <span title={tip} style={{ display: 'inline-flex', alignItems: 'center', gap: 5, cursor: 'help' }}>
            {dec.length ? chip(`결정 ${nfmt(dec.length)}`, '#2f8f5b', 'rgba(47,143,91,.10)') : null}
            {act.length ? chip(`조치 ${nfmt(act.length)}`, '#d98a16', 'rgba(217,138,22,.12)') : null}
